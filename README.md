...
> **R:** Porque el `#include` copia el contenido entero de los headers del sistema (stdio.h, stdlib.h), que tienen cientos de declaraciones y macros propias. El fuente tiene 94 líneas pero los headers que incluye suman mucho más.
...
> **R:** El preprocesador elimina todos los comentarios antes de generar el `.i`. No tienen ningún significado para el compilador así que los borra.
...
> **R:** Sin `-DDEBUG`: no devuelve nada (LOG se define como vacío, la línea desaparece). Con `-DDEBUG`: `    printf("[DEBUG] %s\n", ("Iniciando main"));`
...
> **R:** Indican de qué archivo y línea viene el código que sigue, para que el compilador pueda reportar errores apuntando al fuente original y no al `.i`. La declaración de `printf` viene de `stdio.h` del sistema (en mi caso `C:/TDM-GCC-64/x86_64-w64-mingw32/include/stdio.h`).
...
> **R:** Guardan el estado del stack frame anterior (`pushq %rbp`) y configuran uno nuevo para la función (`movq %rsp, %rbp`). Básicamente preparan el entorno de la función antes de ejecutar su código. Las directivas `.seh_*` son específicas de Windows.
...
> **R:** Porque `programa.c` llama a `area_circulo` pero no la define, solo tiene el prototipo del `.h`. El compilador sabe que existe pero no tiene el código. En `matematica.c` sí está la implementación completa, por eso aparece como `T`.
...
> **R:** `cannot execute binary file: Exec format error`. No es un ejecutable, le faltan los símbolos resueltos y el punto de entrada que agrega el enlazador.
...
> **R:** Quedan porque son funciones de bibliotecas dinámicas del sistema que no se copian al ejecutable. Se resuelven en tiempo de ejecución cuando el SO carga el programa y busca las DLLs necesarias.
...
> **R:** `CUADRADO(x)` es una sustitución de texto pura: el preprocesador reemplaza `CUADRADO(5)` por `((5) * (5))` antes de compilar, así que desaparece en el **preprocesamiento** y no hay verificación de tipos. `sumar(a, b)` en cambio es una función real: el compilador verifica los tipos, genera una instrucción `call` en el ensamblador, y desaparece (en el sentido de convertirse en código máquina) en la **compilación**.
...
> **R:** `T` es para código (funciones), vive en la sección `.text`. `D` es para variables globales con valor inicial, vive en `.data`. Básicamente uno son instrucciones de máquina y el otro son datos. Por ejemplo `sumar` es `T` y una variable global `int x = 5;` sería `D`.
...
> **R:** Estoy en Windows así que no tengo `ldd` ni `otool`. Con `objdump -p programa.exe | grep "DLL"` se puede ver que el ejecutable depende de `msvcrt.dll`, `kernel32.dll`, etc.

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque `gcc` la incluye solo. Todo programa C la necesita para cosas como `printf` y el startup code, así que `gcc` la agrega automáticamente al invocar al enlazador.
